...
A plataforma escolhida é a AWS, combinando um balanceador de carga para receber o tráfego HTTPS com a execução da API em contêineres sem servidor (ECS Fargate) e um SQL Server gerenciado (Amazon RDS). Essa composição reduz a sobrecarga operacional — não precisamos cuidar de sistemas operacionais ou patches de banco — e facilita a escalabilidade conforme as rotas de busca de professores e os picos de agendamento crescem. O tráfego de entrada é encerrado em HTTPS no balanceador, usando certificados emitidos e renovados automaticamente, e segue por uma rede privada até as tarefas da API e, destas, ao banco.
